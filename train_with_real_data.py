#!/usr/bin/env python3
"""Train TAN model with real Retailrocket data."""
...
def check_data_exists():
    ...
    data_path = Path('data/raw/ecommerce/retailrocket/events.csv')
    if not data_path.exists():
        print("❌ Real data not found!")
        print(f"   Expected: {data_path}")
        print("\n📥 Download real data first:")
        print("   nacf-download-data --dataset retailrocket")
        print("\n   Or use synthetic data by running:")
        print("   python experiments/hypothesis_testing/h1_friction_detection/train_tan.py")
        return False
    return True
...
def main():
    ...
    try:
        ...
        if len(df) < 100000:  # Real Retailrocket has 2.7M events
            print("⚠️  Warning: Dataset seems small. May be synthetic data.")
        else:
            print(f"✓ Loaded {len(df):,} events (real data)")
    ...
    # Save results
    results_file = Path('results/real_data_training_results.txt')
    with open(results_file, 'w') as f:
        f.write("TAN Training Results (Real Retailrocket Data)\n")
        f.write("=" * 70 + "\n\n")
        f.write(f"Dataset: {len(df):,} events, {len(sessions_df):,} sessions\n")
